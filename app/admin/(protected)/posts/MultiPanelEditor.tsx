...
export default function MultiPanelEditor({
    allVersions,
    defaultLang,
    baseData,
}: Props) {
    ...
    const [panelStates, setPanelStates] =
        useState<Record<string, PanelState>>({});
    ...
    return (
        <div>
            ...
            <div className="sticky top-0 z-20 mb-6 flex flex-wrap items-center gap-2 rounded-xl border border-blue-100 bg-blue-50 p-3 shadow-sm dark:border-blue-900/30 dark:bg-blue-900/10">
                ...
                {LOCALES.map((loc) => {
                    ...
                    return (
                        ...
                        >
                            {exists
                                ? `Edit ${LOCALE_LABELS[loc]}`
                                : `+ Add ${LOCALE_LABELS[loc]}`}
                        </button>
                    );
                })}

                {/* Divider */}
                <div className="mx-1 h-5 w-px bg-blue-200 dark:bg-blue-800" />
                ...
                <button
                    ...
                    className="rounded-lg border border-gray-300 bg-white px-4 py-1.5 text-sm font-semibold text-gray-600 transition hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-300 dark:hover:bg-gray-700"
                >
                    Cancel
                </button>
                ...
                <Link
                    href="/admin/posts"
                    className="ml-auto rounded-lg border border-gray-300 px-4 py-2 text-sm text-gray-600 hover:bg-gray-50 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-800"
                >
                    ← Back to Posts
                </Link>
            </div>
            ...
            {/* Panels */}
            <div
                className={`flex gap-6 overflow-x-auto ${isSingle ? 'justify-center' : ''}`}
            >
                {openLangs.map((lang) => {
                    const panelData =
                        getPanelData(lang);
